(function ($) {
    ...
    $('.email-list').length && $.ajax({
        dataType: "json",
        url: "/pages/js/pages.projetos.body.js",
        success: function (data) {
            $.each(data.emails, function (i) {
                ...
                $.each(list, function (j) {
                    ...
                    var rodovias = $this.rodovias;
                    var indice= $this.indice;
                    var indiceac = $this.indiceac;
                    var inicio = $this.inicio;
                    var termino = $this.termino;
                    ...
                    var li = '<li class="item padding-15" data-email-id="' + id + '"> \
                                ...
                                <div class="inline m-l-15"> \
                                    <p class="processo no-margin hint-text small">' + projeto + '</p> \
                                    <p class="obrigacao no-margin">' + titulo + '</p> \
                                    <p class="funcao no-margin">' + empresa + '</p> \
                                </div> \
    ...
    $('body').on('click', '.item', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                emailOpened.find('.projeto').text(email.projeto);
                emailOpened.find('.titulo').text(email.titulo);
                emailOpened.find('.descricao').text(email.descricao);
                emailOpened.find('.empresa').text(email.empresa);
                emailOpened.find('.rodovias').text(email.rodovias);
                emailOpened.find('.indice').text(email.indice);
                emailOpened.find('.indiceac').text(email.indiceac);
                emailOpened.find('.inicio').text(email.inicio);
                emailOpened.find('.termino').text(email.termino);
                emailOpened.find('.prazo').text(email.prazo);
                emailOpened.find('.prazo').text(email.prazo);
                emailOpened.find('.situacao').text(email.prsituacaoazo);
