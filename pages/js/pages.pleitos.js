(function ($) {
    ...
    $('.email-list').length && $.ajax({
        dataType: "json",
        url: "/pages/js/pages.pleitos.body.js",
                ...
                $.each(list, function (j) {
                    ...
                    var li = '<li class="item padding-15" data-email-id="' + id + '"> \
                                ...
                                <div class="inline m-l-15"> \
                                    ...
                                    <p class="descricao no-margin">' + descricao + '</p> \
    ...
    $('body').on('click', '.item', function (e) {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                emailOpened.find('.tipo').text(email.tipo);
                ...
                emailOpened.find('.protocolo').text(email.protocolo);
                emailOpened.find('.abertura').text(email.abertura);
                emailOpened.find('.situacao').text(email.situacao);
                emailOpened.find('.orgao').text(email.orgao);
                emailOpened.find('.duedate').text(email.duedate);

                var obrigacoestext = function () {
                    var htmloutput = "";

                    htmloutput = "<div class=\"form-group\"><div class=\"\"><table class=\"table m-t-10\"><thead>";

                    $.each(email.obrigacao, function (index, value) {

                        htmloutput += "<tr><th class=\"\">";

                        htmloutput += "<label class=\"text-black col-sm-height \" \">&nbsp;&nbsp;" + value + "</label>";
                ...
                emailOpened.find('.obrigacoes').html(obrigacoestext);
